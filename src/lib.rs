...
extension!(
    ...
    js = ["crypto_key_symbol.js" =
        { source = "__bootstrap.internals.kKeyObject = Symbol('kKeyObject');" },],
);
...
fn deno_web_with_dom_events(
    ...
    let mut extension = deno_web::deno_web::init(blob_store, None, false, broadcast_channel);
...
fn deno_fetch_extension() -> deno_core::Extension {
...
impl Renderer {
    ...
    fn replace_document(
        ...
        self.images_nodes_loaded.clear();
        self.pending_canvas_update = false;
        self.animations.clear();
        // Node IDs are reused by the new document; visibility belongs to the old one.
        self.tracking_intersection.clear();
        self.nodes_intersecting.clear();
...
impl Frame {
  ...
  <body></body>
    ...
    fn register_tokio_runtime(&mut self) -> Result<()> {
    ...
    fn open(&mut self) -> Result<BootParams> {
    ...
    fn handle_main_event(&mut self, proxy: &RendererProxy, event: FrameCommand) {
        match event {
            FrameCommand::Render => {
...
/// Run the desktop browser's command-line entry point.
pub fn run() -> Result<()> {
...
mod tests {
    ...
    #[test]
    fn headless_frame_reports_startup_errors() {
        assert!(Frame::open_headless("not a URL".into()).is_err());
    }

    #[test]
    fn headless_frame_pumps_events_and_stops_on_drop() -> Result<()> {
        ...
        let server = thread::spawn(move || -> Result<()> {
            ...
            loop {
                let (mut stream, _) = match listener.accept() {
                    ...
                    Err(error) if error.kind() == ErrorKind::WouldBlock => {
                        if Instant::now() >= deadline {
                            bail!("Headless frame did not run its animation callback");
                        }
                ...
                let callback = request.starts_with("GET /tick ");
                let body = if callback {
                    ...
                    setTimeout(async () => {
                        ...
                        if (exported.byteLength !== 16) throw new Error('Crypto key round trip failed');

                        const workerUrl = URL.createObjectURL(new Blob([
                            'self.onmessage = ({ data }) => self.postMessage(data + 1);'
                        ...
                        if (reply !== 42) throw new Error('Worker messaging failed');

                        document.body.textContent = 'updated';
                        requestAnimationFrame(() => fetch('/tick'));
                    }, 50);
                    </script></body></html>"#
                };
                write!(
        ...
        let frame = Frame::open_headless(url)?;
        let progress = progress_rx.recv_timeout(Duration::from_secs(10));
        let tx = frame.tx.clone();
        drop(frame);

        // Shutdown must reach the event loop even when its JS runtime is idle.
        let deadline = Instant::now() + Duration::from_secs(5);
        while tx.send(FrameCommand::Render).is_ok() {
            assert!(Instant::now() < deadline, "Headless frame failed to stop");
            thread::sleep(Duration::from_millis(5));
        }
